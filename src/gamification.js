...
const ACHIEVEMENTS = {
  ...
  first_heartbeat: {
    ...
    description: '第一次心跳加速',
    icon: '💓',
    condition: (state) => state.dynamicState?.closeness >= 80,
    reward: { closeness: 5 },
    rarity: 'uncommon'
  },
  trust_fall: {
  ...
  soulmate: {
    ...
    condition: (state) => {
      ...
      return ds?.closeness >= 80 &&
             ds?.trust >= 80 &&
             ds?.security >= 80 &&
             ds?.neediness >= 80 &&
             ds?.possessiveness >= 80;
    },
    reward: { trust: 5, security: 5, closeness: 5, neediness: 5, possessiveness: 5 },
    rarity: 'legendary'
  },
