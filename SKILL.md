...
- **image-api**: [harrylarryxyz/image-api](https://github.com/harrylarryxyz/image-api) — 通用图片生成与编辑，支持任意 OpenAI 兼容 provider。安装：`git clone` 后将 `scripts/`、`references/`、`SKILL.md` 复制到 `~/.hermes/skills/image-api/`。需要 `IMAGE_API_KEY` + `IMAGE_API_BASE` 环境变量。
